...
void editorRowDelChar(erow *row, int);
...
int getWindowSize(int *rows, int *cols) {
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
        if (write(STDOUT_FILENO, "\033[999C\033[999B", 12) != 12) return -1;
...
void editorRefreshScreen() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "\033[%d;%dH", (E.cy-E.rowoff)+1, (E.rx-E.coloff)+1);
...
int main(int argc, char *argv[]) {
    ...
    editorSetStatusMessage("HELP: Ctrl-Q = quit | Ctrl-Q = quit | Ctrl-F = find");
